...
class Recall(APIView):
    ...
    def get(self, request, geography_id, format=None):
        if Encounter.objects.select_related("geography").filter(
            geography__id=geography_id
        ):
            encounter_obj = Encounter.objects.select_related("geography").filter(
                geography__id=geography_id
            )
            serializer = RecallSerializer(
